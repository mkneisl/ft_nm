...
bool MARCH_FUNC(verifyHeader)(t_elf_map* elfMap)
{
    ElfV_Ehdr* elfHdr;
...
bool MARCH_FUNC(mapSectionHeaders)(t_elf_map* elfMap)
{
...
t_mapped_section* MARCH_FUNC(mapSection)(t_elf_map* elfMap, ElfV_Shdr* section)
{
    ...
    if (elfMap->largeFile)
    {
        ...
        mappedSection->data = APPLY_OFFSET(elfMap->fileHdrRange, section->sh_offset);
